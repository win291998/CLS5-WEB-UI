...
function getListSurvey() {
  MethodsUtil.requestApiCustom(QuestionService.GetListApprove, TYPE_REQUEST.GET, queryParams).then((result: Any) => {
...
function handleApprove() {
  ...
  MethodsUtil.requestApiCustom(QuestionService.PostAgreeSurvey, TYPE_REQUEST.POST, payload).then((result: Any) => {
    getListSurvey()
...
function handleReject() {
  ...
  MethodsUtil.requestApiCustom(QuestionService.PostRejectSurvey, TYPE_REQUEST.POST, payload).then((result: Any) => {
    getListSurvey()
    toast('SUCCESS', t(result.message))
    selected.value = []
  }).catch((err: Any) => {
...
<template>
  ...
  <div class="mt-2 mb-3">
    <CpHeaderAction
      v-model:keyword="queryParams.searchData"
      v-model:page-number="queryParams.pageNumber"
      v-model:page-size="queryParams.pageSize"
