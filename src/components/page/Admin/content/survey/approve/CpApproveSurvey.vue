...
function getListSurvey() {
  MethodsUtil.requestApiCustom(QuestionService.GetListSurvey, TYPE_REQUEST.GET, queryParams).then((result: Any) => {
...
function handleApprove() {
  const payload = {
    listId: selected.value.map(i => i.id),
  }
  MethodsUtil.requestApiCustom(QuestionService.PostApproveQuestionSurvey, TYPE_REQUEST.POST, payload).then((result: Any) => {
    getListSurvey()
...
function handleReject() {
  const payload = {
    listId: selected.value.map(i => {
      return {
        id: i.id,
        keyNote: i.description,
      }
    }),
  }
  MethodsUtil.requestApiCustom(QuestionService.PostDeclineQuestionSurvey, TYPE_REQUEST.POST, payload).then((result: Any) => {
    getListSurvey()
    toast('SUCCESS', t(result.message))
    selected.value = []
  }).catch((err: Any) => {
